...
ggplot(data = threat_percent_anxiety_binned_df, mapping = aes(x = anxiety_binned, y = percentage_chose_threat, fill = anxiety_binned)) +
  ...
  labs(title = "Percentage of Trials Threat Valence Image Incorrectly Chosen", x = " Trait Anxiety Level", y = "Percentage", color = "Anxiety Level")+
...
ggplot(STAI_binned_acc_ooo, aes(x = chosenThreatStatus, y = mean_acc, fill = anxiety_binned))+
  ...
  labs(title = "Mean Accuracy For All Trials", x = "Chosen Threat Status", y = "Mean Accuracy") +
  scale_fill_manual(name = "Anxiety Level",
                    values = c("high trait anxiety" = "red", "low to moderate trait anxiety" = "blue"))
```
...
ggplot(dprime_anxiety_binned_df, aes(x = transition_threat, y = dprime, fill = anxiety_binned)) +
  ...
  scale_x_discrete(labels = c("contains_threat" = "Involves Threat", "no_threat" = "Does Not Involve Threat"))
...
ggplot(check_answer_percent_STAI_binned, aes(x = threat_status, y = percentage, fill=anxiety_binned)) +
  ...
  #geom_jitter(width=0.1) +
  geom_point(position = position_jitterdodge(jitter.width = 0.1)) +
  labs(title = "Percentage of Trials Image Valence Type Learned First", x = "Threat Status", y = "Percentage",
       fill = "Anxiety Level") +
  scale_fill_manual(values = c("high trait anxiety" = "red", "low to moderate trait anxiety" = "blue"),
    labels = c("High Trait Anxiety", "Low to Moderate Trait Anxiety"))
...
For the vast majority of trials, each person got threat and neutral images all correct within the same trial, and the image valences were learned at "equal" rates.
For trials in which threat and neutral images were not learned within the same "equal" trial, the percentage of trials each participant learned all threat images first is significantly larger than the percentage they learned all neutral images first.
Anxiety level had no significant effect on which image valence was learned first. 
...
plot_avg <- plot_avg +
  labs(colour = "Image Valence", title = "Learning Trajectories for Initial Learning Phase Task")
plot_avg
```
...
Indiidual learning trajectories differ in the initial learning phase task.
